...
function launch_cluster() {
  ...
  sudo ip netns exec "$namespace" bash -c "MON=1 OSD=1 MGR=1 MDS=0 RGW=0 ${CEPH_SRC}/mstart.sh $namespace --short -n -d --without-dashboard"
}
...
function config_cluster() {
  ...
  sudo ip netns exec "$namespace" ceph -c "$CEPH_CONF" osd pool create pool1
  sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" pool init pool1

  if [ "$designation" == "primary" ] ; then
    sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" mirror pool enable pool1 image
  
    #create token
    sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" mirror pool peer bootstrap create pool1 | tail -n 1 > token
  
    sudo ip netns exec "$namespace" rbd-mirror --cluster "$namespace" --rbd-mirror-delete-retry-interval=5 --rbd-mirror-image-state-check-interval=5 --rbd-mirror-journal-poll-age=1 --rbd-mirror-pool-replayers-refresh-interval=5 --debug-rbd=40 --debug-journaler=40 --debug-rbd_mirror=40 --daemonize=true
  
    sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" create pool1/test-demote-sb --size 100G --image-feature layering --image-feature exclusive-lock --image-feature object-map --image-feature fast-diff --debug-ms 0 --debug-rbd 20 --debug-rbd-mirror 20

    sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" mirror image enable pool1/test-demote-sb snapshot
  fi

  if [ "$designation" == "secondary" ] ; then
    sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" mirror pool peer bootstrap import pool1 token

    sudo ip netns exec "$namespace" rbd-mirror --cluster "$namespace" -c "$CEPH_CONF" --rbd-mirror-delete-retry-interval=5 --rbd-mirror-image-state-check-interval=5 --rbd-mirror-journal-poll-age=1 --rbd-mirror-pool-replayers-refresh-interval=5 --debug-rbd=20 --debug-rbd-mirror=20 --daemonize=true
  fi
}
...
function launch_io() {
  local namespace=$1
  CEPH_CONF=run/"$namespace"/ceph.conf
  bdev=$(sudo ip netns exec "$namespace" rbd -c "$CEPH_CONF" device map -o noudev pool1/test-demote-sb)
  sudo mkfs.xfs "$bdev"
...
function create_mirror_snap() {
  ...
  sudo ip netns exec "$namespace" rbd --cluster "$namespace" mirror image snapshot pool1/test-demote-sb
}

function wait_for_new_snap() {
  local namespace=$1

  OUTPUT=$(sudo ip netns exec "$namespace" rbd --cluster "$namespace" mirror image status pool1/test-demote-sb | grep bytes_per_second| awk '{print $3}')
...
function tear_down() {
  ...
  sudo ip netns exec "$namespace1" rbd --cluster sitea device unmap -o noudev pool1/test-demote-sb
