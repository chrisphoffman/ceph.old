...
inline const char *mds_metric_name(mds_metric_t m)
{
  ...
  case MDS_HEALTH_XATTR_SELINUX: return "MDS_XATTR_SELINUX";
...
inline const char *mds_metric_summary(mds_metric_t m)
{
  ...
  case MDS_HEALTH_XATTR_SELINUX:
    return "%num% MDS report excessive selinux setxattr events";
  default:
    return "???";
  }
}
